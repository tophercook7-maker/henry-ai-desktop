...
// Mirror of the message-content convention documented at the top of
// session_store.py. A message's `content` is either a plain string OR a list of
// typed blocks, so one message can represent tool use and agent actions — not
// just text. Use these builders everywhere (chat, tool runner, scheduler, the
// future computer-use layer) so the whole corpus shares one shape; the Python
// store flattens block text into messages.content_text for FTS.
