...
/* ── PAIR SCREEN ─────────────────────────────────────────────── */
#pair-screen{position:fixed;inset:0;z-index:200;background:var(--bg);display:none;flex-direction:column;align-items:center;justify-content:center;gap:20px;padding:40px 28px;text-align:center}
...
<div id="pair-screen" style="padding-top: max(44px, env(safe-area-inset-top, 44px))">
  ...
  <button class="pair-btn" id="open-companion-btn">Open Companion →</button>
</div>
...
window.addEventListener('load', async () => {
  ...
  // Auto-show pair screen ONLY when not at the Henry server directly
  ...
  // Load today's data with explicit error catch for debugging
  (async function() {
    try {
      await loadToday();
    } catch(loadErr) {
      console.error('[Henry] loadToday error:', loadErr && loadErr.message, loadErr);
    }
  })();
  setInterval(function() {
    loadToday().catch(function(e) { console.warn('[Henry] loadToday poll error:', e && e.message); });
  }, 15000);
  refreshScreen();
  ...
  // Wire Open Companion button — use id not onclick to avoid CSP issues
  const _ocBtn = document.getElementById('open-companion-btn');
  if (_ocBtn) _ocBtn.addEventListener('click', hidePair);
  window.hidePair = hidePair;

  // Expose all companion functions globally so onclick= attrs work
  // (all functions are closure-scoped inside window.addEventListener('load'))
  window.sendMsg = sendMsg;
  window.sendPrompt = sendPrompt;
  window.toggleHabit = toggleHabit;
  window.showTab = showTab;
  window.phoneTo = phoneTo;
  window.macKey = macKey;
  window.henryVPN = henryVPN;
  window.henryVPNOff = henryVPNOff;
  window.refreshScreen = refreshScreen;
  window.loadToday = loadToday;

  // Wire send button with addEventListener (more reliable than onclick=)
  const _sb2 = document.getElementById('send-btn');
  if (_sb2) _sb2.addEventListener('click', sendMsg);
...
function hidePair() {
  const _ps = document.getElementById('pair-screen');
...
async function loadToday() {
  try {
    const resp = await fetch(BASE + '/sync/mac/today', {cache:'no-store', signal: AbortSignal.timeout(8000)});
    ...
    console.warn('[Henry] loadToday failed:', e.message);
...
// ── AUTO-INIT: render data immediately outside closure scope ─────────────────
(function autoInit() {
  var _BASE = location.origin;
  function _ri() {
    fetch(_BASE + '/sync/mac/today', {cache: 'no-store', signal: AbortSignal.timeout(8000)})
      .then(function(r) { return r.json(); })
      .then(function(d) {
        var habits = d.habits || [], logs = d.habitLogs || [], tasks = d.tasks || [], rems = d.reminders || [];
        // Habits
        var hl = document.getElementById('habit-list');
        var hp = document.getElementById('habit-prog');
        if (hl && habits.length) {
          var done = habits.filter(function(h) { return logs.find(function(l) { return l.habit_id === h.id; }); }).length;
          if (hp) hp.textContent = done + '/' + habits.length;
          hl.innerHTML = habits.map(function(h) {
            var isDone = !!logs.find(function(l) { return l.habit_id === h.id; });
            return '<div class="habit-row">' +
              '<div class="habit-check ' + (isDone ? 'done' : '') + '" onclick="(window.toggleHabit||function(){})(\'' + h.id + '\',' + isDone + ')">' +
              (isDone ? '' : (h.icon || '\u25cb')) +
              '</div><span class="habit-name">' + h.name + '</span>' +
              (isDone ? '<span class="habit-streak">\u2713</span>' : '') + '</div>';
          }).join('');
        }
        // Tasks
        ...
        if (tl) {
          tl.innerHTML = !tasks.length
            ? '<div style="padding:14px 16px;color:var(--muted);font-size:13px">All clear \u2713</div>'
            : tasks.slice(0, 8).map(function(t) {
                var col = t.priority >= 3 ? '#ef4444' : t.priority === 2 ? '#f59e0b' : 'var(--accent)';
                return '<div class="task-row"><div class="task-dot" style="background:' + col + '"></div><span class="task-title">' + t.title + '</span></div>';
              }).join('');
        }
        // Reminders
        var rl = document.getElementById('rem-list');
        if (rl) {
          rl.innerHTML = !rems.length
            ? '<div style="padding:14px 16px;color:var(--muted);font-size:13px">Nothing due</div>'
            : rems.slice(0, 5).map(function(r) {
                return '<div class="rem-row"><span class="rem-icon">\u25ce</span><span class="rem-title">' + r.title + '</span></div>';
              }).join('');
        }
      })
      .catch(function() { setTimeout(_ri, 1500); });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _ri);
  } else {
    _ri();
  }
  setInterval(_ri, 15000);
  // Wire send button (fallback in case closure wiring fails)
