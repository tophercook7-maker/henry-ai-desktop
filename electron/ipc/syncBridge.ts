...
`;
  ...
  if (path === '/sync/prompt' && req.method === 'POST') {
    ...
    // P_TIMER: Job timer — "start timer on J-XXXXX" / "how long on J-XXXXX"
    {
      const _jtStartM = lowerText.match(/^(?:start(?: the)? (?:job )?timer|clock in|begin work)(?: (?:on|for))? ([j]-\d{4,6})/i)
                     || lowerText.match(/^([j]-\d{4,6}) (?:start|clock in|begin)/i);
      if (_jtStartM) {
        const _jtNum = (_jtStartM[1]||_jtStartM[2]||'').toUpperCase();
        const _jtJob = dbGetOne('SELECT id,job_number,client_name,title FROM jobs WHERE UPPER(job_number)=? LIMIT 1', _jtNum) as any;
        if (_jtJob) {
          dbRun("INSERT OR REPLACE INTO settings (key,value) VALUES (?,?)", 'active_timer_job', _jtNum);
          dbRun("INSERT OR REPLACE INTO settings (key,value) VALUES (?,?)", 'active_timer_start', new Date().toISOString());
          sendReply('\u23F1\uFE0F Timer started for **' + _jtNum + '** (' + _jtJob.client_name + ': ' + _jtJob.title.slice(0,30) + ')\n\nSay **"stop timer"** when done or **"how long on ' + _jtNum + '"** to check.');
          return;
        }
      }
      const _jtCheckM = lowerText.match(/^(?:how long|elapsed|time)(?: (?:on|for|have i been on|have i worked on))? ([j]-\d{4,6})/i)
                     || lowerText.match(/^([j]-\d{4,6}) (?:time|elapsed|how long)/i)
                     || lowerText === 'check timer' || lowerText === 'timer status' || lowerText === 'how long on job';
      if (_jtCheckM) {
        const _jtActiveNum = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_job'") as any)?.value||'';
        const _jtActiveStart = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_start'") as any)?.value||'';
        const _jtCheckArr = Array.isArray(_jtCheckM) ? _jtCheckM : [];
        const _jtQueryNum = ((_jtCheckArr[1]||_jtCheckArr[2])||'').toUpperCase() || _jtActiveNum;
        if (_jtQueryNum.startsWith('J-') && _jtActiveStart && _jtQueryNum === _jtActiveNum) {
          const _jtElapsed = Math.round((Date.now() - new Date(_jtActiveStart).getTime()) / 60000);
          const _jtH = Math.floor(_jtElapsed/60); const _jtM = _jtElapsed % 60;
          sendReply('\u23F1\uFE0F **' + _jtQueryNum + '** timer running: **' + (_jtH>0?_jtH+'h ':'')+_jtM+'min**\n\nSay **"stop timer"** to log the hours.');
        } else if (_jtQueryNum.startsWith('J-')) {
          sendReply('No active timer for ' + _jtQueryNum + '. Say **"start timer on ' + _jtQueryNum + '"** to begin.');
        } else {
          sendReply('No active timer. Say **"start timer on J-XXXXX"** to begin tracking a job.');
        }
        return;
      }
      const _jtStopM = lowerText === 'stop timer' || lowerText === 'clock out' || lowerText === 'stop job timer' || lowerText === 'end timer' || /^(?:stop|end|finish)(?: the)? (?:job )?timer$/.test(lowerText);
      if (_jtStopM) {
        const _jtStopNum = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_job'") as any)?.value||'';
        const _jtStopStart = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_start'") as any)?.value||'';
        if (_jtStopNum && _jtStopStart) {
          const _jtMins = Math.round((Date.now() - new Date(_jtStopStart).getTime()) / 60000);
          const _jtHrs = _jtMins / 60;
          const _jtJob2 = dbGetOne('SELECT id,job_number,client_name,title FROM jobs WHERE UPPER(job_number)=? LIMIT 1', _jtStopNum) as any;
          dbRun("DELETE FROM settings WHERE key IN ('active_timer_job','active_timer_start')");
          if (_jtJob2 && _jtHrs >= 0.1) {
            const _jtRate = parseFloat((dbGetOne("SELECT value FROM settings WHERE key='hourly_rate'") as any)?.value||'0');
            const _jtCost = _jtHrs * _jtRate;
            dbRun('INSERT INTO job_log (id,job_id,note,created_at) VALUES (?,?,?,?)', Date.now().toString(36)+Math.random().toString(36).slice(2), _jtJob2.id, 'Timer: '+_jtHrs.toFixed(2)+'h'+(_jtRate>0?' @ $'+_jtRate+' = $'+_jtCost.toFixed(2):''), new Date().toISOString());
            if (_jtCost>0) dbRun('UPDATE jobs SET material_cost=COALESCE(material_cost,0)+?,updated_at=? WHERE id=?', _jtCost, new Date().toISOString(), _jtJob2.id);
            const _jtH2 = Math.floor(_jtMins/60); const _jtM2 = _jtMins%60;
            sendReply('\u23F1\uFE0F Timer stopped — **' + _jtStopNum + '**: ' + (_jtH2>0?_jtH2+'h ':'')+_jtM2+'min logged' + (_jtRate>0?' · **$'+_jtCost.toFixed(2)+'**':''));
          } else {
            sendReply('\u23F1\uFE0F Timer stopped' + (_jtMins ? ' — '+_jtMins+' minutes' : '') + '.');
          }
        } else {
          sendReply('No active timer to stop.');
        }
        return;
      }
    }

    // P_TIMER: Job timer — track elapsed time on a job
    {
      const _jtStartM = lowerText.match(/^(?:start(?: a| the)? (?:job )?timer)(?: (?:on|for))? ([j]-\d{4,6})/i)
                     || lowerText.match(/^([j]-\d{4,6}) (?:start|clock in|start timer)/i)
                     || lowerText.match(/^(?:clock in|begin work)(?: on)? ([j]-\d{4,6})/i);
      if (_jtStartM) {
        const _jtNum = (_jtStartM[1]||_jtStartM[2]||'').toUpperCase();
        const _jtJob = dbGetOne('SELECT id,job_number,client_name,title FROM jobs WHERE UPPER(job_number)=? LIMIT 1', _jtNum) as any;
        if (_jtJob) {
          dbRun("INSERT OR REPLACE INTO settings (key,value) VALUES (?,?)", 'active_timer_job', _jtNum);
          dbRun("INSERT OR REPLACE INTO settings (key,value) VALUES (?,?)", 'active_timer_start', new Date().toISOString());
          sendReply('\u23F1\uFE0F Timer started for **' + _jtNum + '** (' + _jtJob.client_name + ': ' + _jtJob.title.slice(0,30) + ')\nSay **\'stop timer\'** when done.');
          return;
        }
      }
      const _jtStopM = /^(?:stop|end|finish)(?: the)? (?:job )?timer$|^(?:stop timer|clock out|end timer)$/.test(lowerText);
      if (_jtStopM) {
        const _jtSNum = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_job'") as any)?.value||'';
        const _jtSStart = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_start'") as any)?.value||'';
        if (_jtSNum && _jtSStart) {
          const _jtMins = Math.round((Date.now()-new Date(_jtSStart).getTime())/60000);
          const _jtHrs = parseFloat((_jtMins/60).toFixed(2));
          const _jtJob2 = dbGetOne('SELECT id,job_number,client_name FROM jobs WHERE UPPER(job_number)=? LIMIT 1', _jtSNum) as any;
          dbRun("DELETE FROM settings WHERE key IN ('active_timer_job','active_timer_start')");
          if (_jtJob2 && _jtHrs >= 0.08) {
            const _jtRate = parseFloat((dbGetOne("SELECT value FROM settings WHERE key='hourly_rate'") as any)?.value||'0');
            const _jtCost = _jtHrs*_jtRate;
            dbRun('INSERT INTO job_log (id,job_id,note,created_at) VALUES (?,?,?,?)', Date.now().toString(36)+Math.random().toString(36).slice(2), _jtJob2.id, 'Timer: '+_jtHrs+'h'+(_jtRate>0?' @ $'+_jtRate+' = $'+_jtCost.toFixed(2):''), new Date().toISOString());
            if (_jtCost>0) dbRun('UPDATE jobs SET material_cost=COALESCE(material_cost,0)+?,updated_at=? WHERE id=?', _jtCost, new Date().toISOString(), _jtJob2.id);
            const _jtH=Math.floor(_jtMins/60); const _jtM=_jtMins%60;
            sendReply('\u23F1\uFE0F Timer stopped — **' + _jtSNum + '**: ' + (_jtH>0?_jtH+'h ':'')+_jtM+'min'+(_jtCost>0?' · **$'+_jtCost.toFixed(2)+'**':'') + '\nLogged to job work record.');
          } else { sendReply('\u23F1\uFE0F Timer stopped.'); }
        } else { sendReply('No active timer. Say **\'start timer on J-XXXXX\'** to begin.'); }
        return;
      }
      const _jtCheckM = lowerText === 'check timer' || lowerText === 'timer status' || lowerText === 'how long on job'
                      || lowerText.match(/^(?:how long|elapsed|timer|how much time)(?: (?:on|for|have i been working on))? ([j]-\d{4,6})/i);
      if (_jtCheckM) {
        const _jtCNum = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_job'") as any)?.value||'';
        const _jtCStart = (dbGetOne("SELECT value FROM settings WHERE key='active_timer_start'") as any)?.value||'';
        if (_jtCNum && _jtCStart) {
          const _jtCMin = Math.round((Date.now()-new Date(_jtCStart).getTime())/60000);
          const _jtCH=Math.floor(_jtCMin/60); const _jtCM=_jtCMin%60;
          sendReply('\u23F1\uFE0F **' + _jtCNum + '** — running: **' + (_jtCH>0?_jtCH+'h ':'')+_jtCM+'min**\nSay **\'stop timer\'** to log the hours.');
        } else { sendReply('No active timer. Say **\'start timer on J-XXXXX\'** to begin.'); }
        return;
      }
    }
    ...
    // P_DND: Do Not Disturb toggle — must be before AI fallback
    {
      const _pdndM = lowerText.match(/^(?:(?:turn |enable |set |activate )?do not disturb|dnd(?: on)?|focus mode(?: on)?)$/i)
                  || lowerText.match(/^(?:turn off|disable) (?:do not disturb|dnd|focus mode?)$/i);
      if (_pdndM) {
        const _pdndOff = /(?:turn off|disable)/i.test(lowerText);
        try {
          const { execSync: _dndx } = await import('child_process') as typeof import('child_process');
          if (IS_MAC) {
    ...
    // P_MAPS: Maps / Navigation — priority dispatch (before AI)
    {
      ...
      if (_pmapsM) {
        const _pmQ = (_pmapsM[1]||'').trim();
        const _pmClient = dbGetOne("SELECT name,notes FROM contacts WHERE LOWER(name) LIKE ? LIMIT 1", '%'+_pmQ.split(' ')[0].toLowerCase()+'%') as any;
        let _pmAddr = _pmQ;
        if (_pmClient?.notes) {
          const _addrL = _pmClient.notes.split('\n').find((l: string) => /\d+.*(?:st|ave|rd|blvd|dr|ln|ct|way|hwy)/i.test(l));
          if (_addrL) _pmAddr = _addrL.trim();
        }
    ...
    // P_MAPS: Maps / Navigation — before AI fallback
    {
      const _pmM = lowerText.match(/^(?:open maps?|navigate|directions?|drive|maps? to|take me to|get me to)(?: to)? (.{3,40})$/i);
      if (_pmM) {
        const _pmQ = (_pmM[1]||'').trim();
        const _pmC = dbGetOne("SELECT name,notes FROM contacts WHERE LOWER(name) LIKE ? LIMIT 1", '%'+_pmQ.split(' ')[0].toLowerCase()+'%') as any;
        let _pmA = _pmQ;
    ...
    // P_DND: Do Not Disturb — priority dispatch
    {
      const _pdndM = /^(?:(?:turn |enable |set |activate )?do not disturb|dnd(?: on)?|focus mode(?: on)?)$/.test(lowerText)
                  || /^(?:turn off|disable) (?:do not disturb|dnd|focus mode?)$/.test(lowerText);
      if (_pdndM) {
        const _pdndOff = /turn off|disable/i.test(lowerText);
        try {
          const { execSync: _dndx } = await import('child_process') as typeof import('child_process');
    ...
    // ── Bid win rate ──────────────────────────────────────────────────────────
    if (/^(?:bid win rate|win rate|what(?:'s| is)(?: my)? win rate|how many bids?(?: do i|did i)? (?:win|close|convert)|bid conversion)/.test(lowerText)) {
      const _wrBids   = (dbGetOne("SELECT COUNT(*) as n FROM jobs WHERE status='bid'") as any)?.n||0;
      const _wrWon    = (dbGetOne("SELECT COUNT(*) as n FROM jobs WHERE status IN ('scheduled','in_progress','complete','invoiced','paid')") as any)?.n||0;
      const _wrLost   = (dbGetOne("SELECT COUNT(*) as n FROM jobs WHERE status='cancelled'") as any)?.n||0;
      const _wrTotal  = _wrWon + _wrLost;
      const _wrPct    = _wrTotal > 0 ? Math.round((_wrWon/_wrTotal)*100) : 0;
      const _wrVal    = (dbGetOne("SELECT COALESCE(SUM(bid_amount),0) as n FROM jobs WHERE status IN ('complete','invoiced','paid')") as any)?.n||0;
      const _wrOpen   = (dbGetOne("SELECT COALESCE(SUM(bid_amount),0) as n FROM jobs WHERE status='bid'") as any)?.n||0;
      sendReply('**\uD83C\uDFAF Bid Win Rate**\n\n' +
        'Won: **' + _wrWon + '** jobs (' + _wrPct + '%)\n' +
        'Lost/cancelled: ' + _wrLost + '\n' +
        'Still open: ' + _wrBids + ' bids ($' + (_wrOpen as number).toFixed(0) + ')\n\n' +
        'Revenue from won bids: **$' + (_wrVal as number).toFixed(0) + '**');
      return;
    }

    // ── Revenue last week / this week ────────────────────────────────────────
    if (/^(?:(?:revenue|income|earnings?|how much)(?: did i| have i)?(?: make|earn|bring in)?)(?: (?:last|this) week|last 7 days?)$|^(?:last|this) week(?:'s)? (?:revenue|earnings?|income)$|^what did i (?:make|earn) (?:last|this) week$/.test(lowerText)) {
      const _isLastWk = /last/.test(lowerText);
      const _wkStart = (() => { const d = new Date(); const day = d.getDay(); const diff = _isLastWk ? day+7 : day; d.setDate(d.getDate()-diff); d.setHours(0,0,0,0); return d.toISOString().slice(0,10); })();
      const _wkEnd   = (() => { const d = new Date(); const day = d.getDay(); const diff = _isLastWk ? day : day-7; d.setDate(d.getDate()+(6-diff)); d.setHours(23,59,59,0); return d.toISOString().slice(0,10); })();
      const _wkJobs = dbGet("SELECT job_number,client_name,title,COALESCE(paid_amount,invoice_amount,bid_amount,0) as amt,status FROM jobs WHERE status IN ('paid','invoiced','complete') AND strftime('%Y-%W',COALESCE(paid_date,invoiced_date,updated_at)) = strftime('%Y-%W',?) ORDER BY updated_at DESC", (_isLastWk ? new Date(new Date().setDate(new Date().getDate()-7)) : new Date()).toISOString()) as any[];
      const _wkRev = _wkJobs.reduce((s: number,j: any)=>s+j.amt, 0);
      const _wkLabel = _isLastWk ? 'Last week' : 'This week';
      if (!_wkJobs.length) { sendReply(_wkLabel + ': no revenue recorded.'); return; }
